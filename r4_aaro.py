...
class Trader:
    ...
    def RainforestResinStrategy(self, position: int, order_depth: OrderDepth) -> List[Order]:
        ...
        # --- sliding window logic (init these in __init__!) ---
        # in __init__: 
        #   self.rain_window = deque(maxlen=10)
        #   self.rain_window_size = 10
        ...
        # 1) Aggressive buys: any ask < pivot
        ...
        # 2) If still flat‑capped, liquidate towards pivot
        ...
        # 3) Fallback buy off the most popular bid
        ...
        # 4) Aggressive sells: any bid > pivot
        ...
        # 5) If still limit‑stuck, liquidate towards pivot
        ...
        # 6) Fallback sell off the most popular ask
...
# on the volatility of the asset we could increase our loss threshold.
    ...
    def KelpMMStrategy(self, position: int, order_depth: OrderDepth) -> List[Order]:
        ...
        - Single‑quote per side
        - Inventory skew
        - Order‑book imbalance shift
        - Window‑based soft/hard liquidation
        """
        product = "KELP"
        limit = self.limits[product]
        orders: List[Order] = []
        ...
        # 1) Top‑of‑book midpoint and volume
        ...
        # 7) Soft liquidation: widen spread
        if soft_liq:
            bid_px -= 1
            ask_px += 1

        # 8) Post buy-side quote (improve best_bid)
        ...
        # 9) Post sell-side quote (improve best_ask)
...
# on the volatility of the asset we could increase our loss threshold.
    ...
    def SquidInkStrategy(self, position: int, order_depth: OrderDepth) -> List[Order]:
        ...
        - Mean at 1850, threshold/standard deviation ±40
        - Only trade when mid moves ≥0 ticks toward the mean
        - Max position 50, hit top‑of‑book
        """
        product = "SQUID_INK"
        limit   = self.limits[product]
        orders: List[Order] = []

        # 1) Ensure both sides of book exist
        if not order_depth.buy_orders or not order_depth.sell_orders:
            return orders

        # 2) Top‑of‑book bid & ask, compute mid
        ...
        # 3) Grab previous mid (if any) to measure momentum
        prev = self.squid_history[-1] if self.squid_history else None

        # 4) Append current mid to history (cap at 100)
        ...
        # 6a) Buy signal: mid < MEAN−THRESHOLD and has *risen* ≥ MOM_MOVE
        if prev is not None and mid < MEAN - THRESHOLD and (mid - prev) >= MOM_MOVE:
            qty = min(limit - position, abs(ask_vol))
            if qty > 0:
                orders.append(Order(product, best_ask, qty))

        # 6b) Sell signal: mid > MEAN+THRESHOLD and has *fallen* ≥ MOM_MOVE
    ...
    def VolcanicRockStrategy(self, position: int, tradingState: TradingState) -> List[Order]:
        ...
        Volcanic Rock momentum:
         - 5‑tick lookback
         - ATR hard‑coded to 300 (for reference)
         - z‑score = (mid – mid_L) / std(last 5 tick moves)
         - ENTRY_Z threshold to trigger
         - Max position = limits['VOLCANIC_ROCK']
        """
